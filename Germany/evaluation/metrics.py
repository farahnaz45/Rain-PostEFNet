...
def compute_ssim(prediction, target):
    ...
    return ssim(prediction, target, data_range=target.max() - target.min())

def compute_lpips(prediction, target):
    """Compute Learned Perceptual Image Patch Similarity (LPIPS)"""
    lpips_metric = LearnedPerceptualImagePatchSimilarity()
    return lpips_metric(torch.tensor(prediction).unsqueeze(0), torch.tensor(target).unsqueeze(0)).item()

def compute_crps(prediction, target):
    """Compute Continuous Ranked Probability Score (CRPS)"""
    return crps_ensemble(target.flatten(), prediction.flatten()).mean()
...
def compile_metrics(dataset: StandardDataset, predictions: np.ndarray, timestamps: List,
    ...
    confusion_matrix = np.zeros((n_classes, n_classes), dtype=np.int32)  # Or np.int64
...
def compute_evaluation_metrics(df: pd.DataFrame):
    ...
    df['far'] = df['fa'] / (df['hit'] + df['fa']+ 1e-8)  # false alarm ratio
...
# def compute_evaluation_metrics(df: pd.DataFrame, predictions, targets):
#     """
#     Compute evaluation metrics in-place for dataframe containing binary confusion matrix.
#     """
#     df['acc'] = (df['hit'] + df['cn']) / (df['hit'] + df['miss'] + df['fa'] + df['cn'])  # accuracy
...
#     df['csi'] = df['hit'] / (df['hit'] + df['miss'] + df['fa'])  # critical success index
#     df['bias'] = (df['hit'] + df['fa']) / (df['hit'] + df['miss'])  # bias
#     df['hss'] = 2 * (df['hit'] * df['cn'] - df['miss'] * df['fa']) / (
#         df['miss']**2 + df['fa']**2 + 2 * df['hit'] * df['cn'] + (df['miss'] + df['fa']) * (df['hit'] + df['cn']))
    
#     # Compute SSIM, LPIPS, and CRPS
#     df['ssim'] = [compute_ssim(p, t) for p, t in zip(predictions, targets)]
#     df['lpips'] = [compute_lpips(p, t) for p, t in zip(predictions, targets)]
#     df['crps'] = [compute_crps(p, t) for p, t in zip(predictions, targets)]
    
#     print(df[['acc', 'pod', 'far', 'csi', 'bias', 'hss', 'ssim', 'lpips', 'crps']])
#     return df
