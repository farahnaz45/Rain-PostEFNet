...
class CustomTensorDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        y_resized = torch.nn.functional.interpolate(
            y.float().unsqueeze(0).unsqueeze(0), size=self.size, mode='bilinear', align_corners=False
        )
...
class CustomTensorDataset_SD(Dataset):
    def __init__(self, input_tensors,gt_tensors):
        ...
        print("shape of input in custom tensor class", input_tensors.shape)
        print("shape of gt in custom tensor class", gt_tensors.shape)
