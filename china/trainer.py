...
class NIMSTrainer_Germnay_Two:
    ...
    def train(self):
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            if val_csi >= val_best_csi:
                ...
                # best_ground_truth = target.detach().cpu().numpy()
            with open(self.log_json_path, 'w') as f:
                json.dump(self.log_dict, f)
    ...
    # def visualize_predictions(self, predictions, ground_truth, num_samples=5, save_path="best_epoch_visualization.png"):
    #     """
    #     Visualizes the best epoch's predictions and corresponding ground truth.
    ...
    #         pred = predictions[i].detach().cpu().numpy()  # Convert to NumPy
    #         gt = ground_truth[i].cpu().numpy()  # Convert to NumPy

    #         # Convert pred from (3, 64, 64) to (64, 64, 3) for RGB visualization
    #         pred = pred.transpose(1, 2, 0)

    #         # Normalize pred if needed (Matplotlib expects values in [0,1] for images)
    #         pred = (pred - pred.min()) / (pred.max() - pred.min())

    #         # Convert ground truth to grayscale (squeeze channel dimension)
    #         gt = gt.squeeze(0)
    ...
    #         axes_row[0].imshow(gt, cmap="gray")
    #         axes_row[0].set_title("Ground Truth")
    #         axes_row[0].axis("off")

    #         # Plot Prediction (RGB)
    #         axes_row[1].imshow(pred)  # No need for cmap, as it's already RGB
    #         axes_row[1].set_title("Prediction (RGB)")
    #         axes_row[1].axis("off")
    ...
    def _epoch(self, data_loader, mode):
        ...
        for i, (images, target, t) in enumerate(pbar):
            ...
            output,output2 = self.model(images, t)
            timestamps = None
            # self.visualize_predictions(output2, target, num_samples=5)
